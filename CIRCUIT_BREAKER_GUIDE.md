...
| 상태 | 의미 | 동작 |
|------|------|------|
| **CLOSED (0)** | 정상 상태 | 모든 요청 통과, 메트릭 기록 |
| **OPEN (1)** | 서비스 장애 감지 | 요청 차단 (CallNotPermittedException), 30초 대기 |
| **HALF_OPEN (2)** | 복구 시도 중 | 제한된 요청으로 서비스 상태 확인 (최대 3개) |
| **DISABLED (3)** | 비활성화 상태 | 항상 요청 통과, 메트릭만 기록 (모니터링 모드) |
| **FORCED_OPEN (4)** | 강제 차단 | 외부에서 수동으로 차단 (관리 목적) |
| **FORCED_HALF_OPEN (5)** | 강제 복구 시도 | 외부에서 수동으로 복구 시도 설정 (테스트 목적) |
