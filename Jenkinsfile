def withStageLogging(Closure body) {
  timestamps {
    ansiColor('xterm') {
      body()
    }
...
for attempt in ${dollar}(seq 1 ${attempts}); do
  cid=${dollar}(${composeCmd} ps -q ${service})
  if [ -z "${dollar}cid" ]; then
    echo "${service}: container not created yet (${dollar}attempt/${attempts})"
  else
    status=${dollar}(docker inspect --format '{{.State.Status}}' ${dollar}cid)
    health=${dollar}(docker inspect --format '{{if .State.Health}}{{.State.Health.Status}}{{else}}none{{end}}' ${dollar}cid)
    if [ "${dollar}status" = "running" ]; then
      if [ "${dollar}health" = "healthy" ] || [ "${dollar}health" = "none" ]; then
        echo "${service}: running (status=${dollar}status health=${dollar}health)"
        exit 0
      fi
    fi
    if [ "${dollar}status" = "exited" ] || [ "${dollar}status" = "dead" ]; then
      echo "${service}: exited unexpectedly (status=${dollar}status health=${dollar}health)"
      ${composeCmd} logs ${service} || true
      exit 1
    fi
    echo "${service}: still starting (status=${dollar}status health=${dollar}health) (${dollar}attempt/${attempts})"
  fi
  sleep 5
done
