...
**분석**: 성능 목표를 단순 달성이 아니라 **8배 초과 달성**. 현재 시스템은 2개 ingest-service 인스턴스가 부하를 나눠서 처리 중이며, K6 부하 테스트 결과 총 800 RPS 달성. 2개 인스턴스가 협력하여 이 수준의 성능을 구현하고 있음.
...
**현재 상태**:
- 2개 ingest-service 인스턴스 동시 운영:
  - VM1: ingest-service (port 8080)
  - VM2: ingest-service (port 8083)
- **실제 측정 처리량**: 800 RPS (K6 테스트 결과, 2개 인스턴스 부하 분산)
- **인스턴스당 평균**: ~400 RPS (부하 분산)
...
### 7.1 목표
결제 처리를 **현재 2개 인스턴스 800 RPS** (부하 분산)에서 **N개 인스턴스 8000+ RPS 총 처리량**으로 확장.

### 7.2 현재 아키텍처 제약

**2개 Ingest-Service 인스턴스 부하 분산 중**:
```
클라이언트 요청 (800 RPS)
           ↓
     API Gateway
...
현재 배포:
- VM1 (172.25.0.37): ingest-service (port 8080)
- VM2 (172.25.0.79): ingest-service (port 8083)
```

**현재 처리량**:
- 실제 측정: 800 RPS (2개 인스턴스 부하 분산)
- 목표: 8000 RPS 달성하려면 10배 이상 확장 필요
...
**현재**: 총 2개 인스턴스 (VM1 1개 + VM2 1개) = 800 RPS (부하 분산)
**목표**: 4-8개 인스턴스
**용량**: 3200-6400 RPS
